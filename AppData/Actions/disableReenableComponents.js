...
module.exports = {
  ...
  subtitle: (values, constants, thisAction) => {
    return `${values.state ? "Re-Enable" : "Disable"} ${thisAction.UI.find(e => e.element == 'typedDropdown').choices[values.target.type].name}${thisAction.UI.find(e => e.element == 'typedDropdown').choices[values.target.type].field ? ` ${values.target.value}` : ""}${values.target.type == 'specific_button' ? ` Button #${values.button_index}` : ""} of ${constants.variable(values.message)}`
  },
  compatibility: ["Any"],
  async run(values, msg, client, bridge) {
    ...
    if (values.target.type == 'specific_button') {
      let button = messageComponents[bridge.transf(values.target.value) - 1].components[bridge.transf(values.button_index) - 1];
